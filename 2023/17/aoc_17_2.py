...
class Vertex:
    ...
    def neighbours(self, width, height):
        if self.direction == Direction.NONE:
            ns = (self.move(d) for d in Direction if d != Direction.NONE)
        elif self.consecutive < 3:
            ns = [self.move(self.direction)]
        elif self.consecutive < 9:
            ns = (self.move(d) for d in self.direction.possible())
        else:
            ns = (self.move(d) for d in self.direction.possible() if d != self.direction)
        yield from (n for n in ns if n.pos.in_bounds(width, height))
